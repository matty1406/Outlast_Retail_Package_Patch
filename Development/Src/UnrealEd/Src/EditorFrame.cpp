...
extern UBOOL IsDirect3D11Supported(UBOOL& OutSupportsD3D10Features, UBOOL& OutSupportsD3D11Features);
#endif
...
WxMainMenu::WxMainMenu()
{
	...
	// File menu
	{
		...
		FileMenu->AppendSeparator();
		FileMenu->AppendCheckItem( IDM_SAVE_AS_RETAIL, TEXT("Save as Outlast Retail Package (v867)"), TEXT("When checked, saved packages are stamped and serialized at the Outlast retail version 867 / licensee 3 / engine 10907 instead of the current engine version.") );
		FileMenu->AppendSeparator();
		...
		UBOOL bSupportsD3D10Features = FALSE;
		UBOOL bSupportsD3D11Features = FALSE;

#if _WINDOWS
		UBOOL bSupportsD3D11RHI = IsDirect3D11Supported( bSupportsD3D10Features, bSupportsD3D11Features );
	...
	// View menu
	{
		...
		ViewMenu->Append( IDM_SHOW_CHECKPOINTS, TEXT("Show Checkpoints"), TEXT("Show Checkpoints") );
		ViewMenu->Append( IDM_SHOW_GAMESTATE, TEXT("Show Game State"), TEXT("Show Game State") );
		ViewMenu->AppendSeparator();
...
BEGIN_EVENT_TABLE( WxEditorFrame, wxFrame )
	...
	EVT_MENU( IDM_SAVE_AS_RETAIL, WxEditorFrame::MenuFileSaveAsRetail )
	EVT_UPDATE_UI( IDM_SAVE_AS_RETAIL, WxEditorFrame::UI_MenuFileSaveAsRetail )
	...
	EVT_MENU( IDM_SHOW_CHECKPOINTS, WxEditorFrame::MenuCheckpoints )
	EVT_MENU( IDM_SHOW_GAMESTATE, WxEditorFrame::MenuGameState )
	...
	EVT_MENU( IDMENU_AlignToLastSelected, WxEditorFrame::MenuAlignToLastSelected )
	EVT_MENU( IDMENU_MoveToGrid, WxEditorFrame::MenuMoveToGrid )
	EVT_MENU( IDMENU_MultiMoveToGrid, WxEditorFrame::MenuMultiMoveToGrid )
	EVT_MENU( IDMENU_MultiMoveToGrid2D, WxEditorFrame::MenuMultiMoveToGrid2D )
	EVT_MENU( IDMENU_FixRoundingErrors, WxEditorFrame::MenuFixRoundingErrors )
	...
	EVT_MENU( IDMENU_ActorPopupConvertSoundEmittingMeshActorToStaticMesh, WxEditorFrame::MenuConvertActors )
	EVT_MENU( IDMENU_ActorPopupConvertStaticMeshToSoundEmittingMeshActor, WxEditorFrame::MenuConvertActors )
...
EVT_MENU( IDM_SYNCCORNERMARKERS, WxEditorFrame::MenuSyncCornerMarkers )
...
{	
	if( bShouldUpdateUI )
	{
		...
		if( MainToolBar )
		{
			...
			TArray<FName>* cpList = AOLCheckpointList::GetCheckpointList();
			MainToolBar->CheckpointListCombo->Clear();
			if (cpList && GWorld->PersistentLevel)
			{	
				FName persistentName(*GWorld->PersistentLevel->GetOutermost()->GetName());

				for (INT i = 0; i < cpList->Num(); i++)
				{
					const FName& cpName = (*cpList)(i);
					AOLCheckpoint* checkpoint = Utils::GetCheckpointFromName(cpName);					
					if (checkpoint && checkpoint->PersistentLevelName == persistentName)
					{
						MainToolBar->CheckpointListCombo->Append( *(*cpList)(i).ToString() );
					}
				}
			}
			else
			{
				MainToolBar->CheckpointListCombo->Append(TEXT("(no checkpoints)"));
			}

			MainToolBar->CheckpointListCombo->SetSelection(0);
...
/**
 * Outlast Retail Package Patch - toggles retail-compatible package versioning.
 * When enabled, subsequent package saves are stamped and serialized at the Outlast
 * retail version (867 / 3 / 10907) instead of the current engine version.
 * See UnObjVer.h / ULinkerSave.
 */
void WxEditorFrame::MenuFileSaveAsRetail( wxCommandEvent& In )
{
	GbForceRetailPackageVersion = In.IsChecked() ? TRUE : FALSE;
	if ( GbForceRetailPackageVersion )
	{
		GForcedPackageFileVersion		= VER_RETAIL_PACKAGE_FILE;
		GForcedPackageLicenseeVersion	= VER_RETAIL_PACKAGE_LICENSEE;
		GForcedEngineVersion			= VER_RETAIL_ENGINE_VERSION;
		warnf( TEXT("Package saving switched to Outlast retail version (%d / %d / engine %d). Only saves made now use this format."),
			GForcedPackageFileVersion, GForcedPackageLicenseeVersion, GForcedEngineVersion );
	}
	else
	{
		warnf( TEXT("Package saving switched back to the current engine version.") );
	}
}

void WxEditorFrame::UI_MenuFileSaveAsRetail( wxUpdateUIEvent& In )
{
	In.Check( GbForceRetailPackageVersion ? true : false );
}
...
void WxEditorFrame::MenuCheckpoints( wxCommandEvent& In )
{
	GUnrealEd->ShowCheckpoints();
}

void WxEditorFrame::MenuGameState( wxCommandEvent& In )
{
	GUnrealEd->ShowGameState();
}
...
#endif
	{
		GUnrealEd->PlayWorldCheckpoint = (const TCHAR*)MainToolBar->CheckpointListCombo->GetValue();
...
void WxEditorFrame::MenuAlignToLastSelected( wxCommandEvent& In )
{
	GUnrealEd->Exec( TEXT("ACTOR ALIGN TOLASTSELECTED") );
}
...
void WxEditorFrame::MenuMultiMoveToGrid( wxCommandEvent& In )
{
	GUnrealEd->Exec( TEXT("ACTOR ALIGN MULTIMOVETOGRID") );
}

void WxEditorFrame::MenuMultiMoveToGrid2D( wxCommandEvent& In )
{
	GUnrealEd->Exec( TEXT("ACTOR ALIGN MULTIMOVETOGRID 2D") );
}

void WxEditorFrame::MenuFixRoundingErrors( wxCommandEvent& In )
{
	GUnrealEd->Exec( TEXT("ACTOR ALIGN FIXROUNDINGERRORS") );
}
...
void WxEditorFrame::MenuConvertActors( wxCommandEvent& In )
{
	...
	const UBOOL bFromStaticMesh = (
		...
		In.GetId() == IDMENU_ActorPopupConvertStaticMeshToSoundEmittingMeshActor || // rcharpentier
		In.GetId() == IDMENU_ActorPopupConvertSoundEmittingMeshActorToStaticMesh); // rcharpentier
	...
	const UBOOL bToStaticMesh = (
		...
		In.GetId() == IDMENU_ActorPopupConvertSoundEmittingMeshActorToStaticMesh);
	...
	const UBOOL bToSoundEmittingMeshActor = (In.GetId() == IDMENU_ActorPopupConvertStaticMeshToSoundEmittingMeshActor);
	...
	if ( GUnrealEd->edactDeleteSelected( FALSE, bIgnoreKismetReferenced ) )
	{
		...
		for( INT i = 0 ; i < ConvertInfo.Num() ; ++i )
		{
			...
			else if (bToSoundEmittingMeshActor) // rcharpentier
			{
				// Make current the level into which the new actor is spawned.
				GWorld->CurrentLevel = Info.SourceLevel;
				AOLSoundEmittingMeshActor* SMActor = CastChecked<AOLSoundEmittingMeshActor>( GWorld->SpawnActor(AOLSoundEmittingMeshActor::StaticClass(), NAME_None, Info.Location, Info.Rotation) );
				SMActor->ClearComponents();
				Info.SetToActor(SMActor, SMActor->StaticMeshComponent);
				SMActor->ConditionalUpdateComponents();
				GEditor->SelectActor( SMActor, TRUE, NULL, FALSE );
...
void WxEditorFrame::MenuSyncCornerMarkers( wxCommandEvent& In )
{
	const FScopedTransaction Transaction(TEXT("SyncCornerMarkers"));

	AStaticMeshActor* masterMesh = NULL;
	TArray<AOLCornerMarker*> masterCornerMarkers;
	TArray<AStaticMeshActor*> slaveMeshes;
	TArray<AOLCornerMarker*> boundCornerMarkers;
	...
	for ( FSelectionIterator It( GEditor->GetSelectedActorIterator() ) ; It ; ++It )
	{
		...
		if (Actor->IsA(AStaticMeshActor::StaticClass()))
		{
			masterMesh = Cast<AStaticMeshActor>(Actor);
		}
		else if (Actor->IsA(AOLCornerMarker::StaticClass()))
		{
			masterCornerMarkers.AddItem(Cast<AOLCornerMarker>(Actor));
		}
	}

	if (!masterMesh || !masterMesh->StaticMeshComponent || !masterMesh->StaticMeshComponent->StaticMesh)
	{
		return;
	}
	...
	FString meshName = masterMesh->StaticMeshComponent->StaticMesh->GetPathName();

	for (FActorIterator It; It; ++It)
	{
		AOLCornerMarker* cornerMarker = Cast<AOLCornerMarker>(*It);
		if (cornerMarker && cornerMarker->BoundStaticMesh == meshName && !masterCornerMarkers.ContainsItem(cornerMarker))
		{
			boundCornerMarkers.AddItem(cornerMarker);
		}

		AStaticMeshActor* staticMeshActor = Cast<AStaticMeshActor>(*It);
		if (staticMeshActor && staticMeshActor != masterMesh && staticMeshActor->StaticMeshComponent && staticMeshActor->StaticMeshComponent->StaticMesh && 
			staticMeshActor->StaticMeshComponent->StaticMesh->GetPathName() == meshName)
		{
			slaveMeshes.AddItem(staticMeshActor);
		}
	}

	//
	// Kill all the existing markers bound to that mesh - no pity
	//
	
	GEditor->SelectNone( FALSE, TRUE );
	if (boundCornerMarkers.Num() > 0)
	{
		for (INT i = 0; i < boundCornerMarkers.Num(); i++)
		{		
			GEditor->SelectActor( boundCornerMarkers(i), TRUE, NULL, FALSE );
		}
		GEditor->edactDeleteSelected( FALSE, TRUE );
	}

	//
	// Cache the relative transforms and bind the masters
	//	

	TArray<FMatrix> relativeXforms;
	FRotationTranslationMatrix masterMeshMtx(masterMesh->Rotation, masterMesh->Location);
	FMatrix invMeshMtx = masterMeshMtx.Inverse();
	for (INT i = 0; i < masterCornerMarkers.Num(); i++)
	{
		FRotationTranslationMatrix markerMtx(masterCornerMarkers(i)->Rotation, masterCornerMarkers(i)->Location);
		FMatrix relMtx = markerMtx * invMeshMtx;
		relativeXforms.AddItem(relMtx);

		masterCornerMarkers(i)->BoundStaticMesh = meshName;
	}
	...
	for (INT i = 0; i < slaveMeshes.Num(); i++)
	{
		AStaticMeshActor* slaveMesh = slaveMeshes(i);
		
		for (INT j = 0; j < masterCornerMarkers.Num(); j++)
		{
			// Set the current level to match this mesh
			ULevel* desiredLevel = slaveMesh->GetLevel();
			GWorld->CurrentLevel = desiredLevel;

			FRotationTranslationMatrix slaveMeshMtx(slaveMesh->Rotation, slaveMesh->Location);
			FMatrix markerMtx = relativeXforms(j) * slaveMeshMtx;
			AOLCornerMarker* newMarker = Cast<AOLCornerMarker>(GWorld->SpawnActor(AOLCornerMarker::StaticClass(), NAME_None, markerMtx.GetOrigin(), markerMtx.Rotator(), NULL, TRUE));
			newMarker->CopyFrom(masterCornerMarkers(j));			
		}
	}
...
{
	FVector StartLocation = GEditor->ClickLocation + GEditor->ClickPlane * 3.0f;
