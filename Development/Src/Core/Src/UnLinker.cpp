			...
			)
		{
			...
			if( Export._Object )
			{
				...
				else
				{
					...
					{
						//>> rcharpentier - fix for cross-referenced swf movies, inspired by https://udn.unrealengine.com/questions/138903/strange-crash-when-launch-with-seekfreeloading.html
						if (Export._Object->_Linker && Export._Object->HasAnyFlags(RF_NeedLoad|RF_NeedPostLoad))
						{
							Export._Object->ConditionalPostLoad();
						}
						//<<
...
,	bIsNextObjectSerializePotentialCrossLevelRef( FALSE )
{
	...
	// Outlast Retail Package Patch - retail format is decided PER PACKAGE, so a filtered
	// -SaveAsRetail leaves every other package this process saves untouched.
	const FString RetailPackageName = LinkerRoot ? LinkerRoot->GetName() : FString(TEXT(""));
	if ( appShouldSaveAsRetailPackage( *RetailPackageName ) )
	{
		// Stamp the header AND drive object-body serialization at the forced (retail)
		// version, so the package header and payload are written in the same format.
		Summary.SetFileVersions( GForcedPackageFileVersion, GForcedPackageLicenseeVersion );
		Summary.EngineVersion =	GForcedEngineVersion;
		SetVer( GForcedPackageFileVersion );
		SetLicenseeVer( GForcedPackageLicenseeVersion );
		debugf( TEXT("Outlast Retail Package Patch: saving '%s' at %d / %d / engine %d."),
			*RetailPackageName, GForcedPackageFileVersion, GForcedPackageLicenseeVersion, GForcedEngineVersion );
	}
	else
	{
