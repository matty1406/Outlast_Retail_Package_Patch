...
#if DINGO
	#include "Engine.h"
	#include "DingoD3DTextureAllocator.h"
#endif
...
//@zombie_ps4_begin - Wildcard support
UBOOL FString::MatchesWildcard(const FString& InWildcard, const UBOOL bIgnoreCase) const
{
	FString Wildcard(InWildcard);
	FString Target(*this);
	INT IndexOfStar = Wildcard.InStr(TEXT("*"), true); // last occurance
	INT IndexOfQuestion = Wildcard.InStr( TEXT("?"), true); // last occurance
	INT Suffix = Max(IndexOfStar, IndexOfQuestion);
	if (Suffix == INDEX_NONE)
	{
		// no wildcards
		if (bIgnoreCase)
		{
			return appStricmp( *Target, *Wildcard ) == 0;
		}
		else
		{
			return appStrcmp( *Target, *Wildcard ) == 0;
		}
	}
	else
	{
		if (Suffix + 1 < Wildcard.Len())
		{
			...
			if (!Target.EndsWith(SuffixString))
			{
				return FALSE;
			}
		...
		INT PrefixIndexOfStar = Wildcard.InStr(TEXT("*")); 
		INT PrefixIndexOfQuestion = Wildcard.InStr( TEXT("?"));
		INT Prefix = Min(PrefixIndexOfStar < 0 ? MAXINT : PrefixIndexOfStar, PrefixIndexOfQuestion < 0 ? MAXINT : PrefixIndexOfQuestion);
		check(Prefix >= 0 && Prefix < Wildcard.Len());
		if (Prefix > 0)
		{
			FString PrefixString = Wildcard.Left(Prefix);
			if (!Target.StartsWith(PrefixString))
			{
				return FALSE;
			}
	...
	if (FirstWild == TEXT('*') || FirstWild == TEXT('?'))
	{
		...
		INT MaxNum = Min(Target.Len(), FirstWild == TEXT('?') ? 1 : MAXINT);
		for (INT Index = 0; Index <= MaxNum; Index++)
		{
			if (Target.Right(Target.Len() - Index).MatchesWildcard(Wildcard, bIgnoreCase))
			{
				return TRUE;
			}
		}
...
#if DINGO_AS_PC
	//todo DINGO as PC ?
	TRAP_NEED_TO_IMPLEMENT_DINGO_AS_PC
#else 
...
#if DINGO
	//Make sure the assert message is output to the log on Dingo as an error.
	FString AssertionString = FString::Printf(TEXT("%s(%i): Assertion failed: %s: %s"), ANSI_TO_TCHAR(File), Line, ANSI_TO_TCHAR(Expr), TempStr);
	appDingoLogExceptionError(*AssertionString);
#endif
...
#if DINGO
	//Make sure the assert message is output the log on Dingo as an error.
	FString AssertionString = FString::Printf(TEXT("%s(%i): Assertion failed: %s: %s"), ANSI_TO_TCHAR(File), Line, ANSI_TO_TCHAR(Expr), TempStr);
	appDingoLogExceptionError(*AssertionString);
#endif
...
{
#if DINGO
	const TCHAR* T = *InString;
	while (appStrncmp(T, TEXT("..\\"), 3) == 0)
	{
		T += 3;
	}
	return T;
...
void FMapPackageFileCache::CachePaths()
{
	...
	debugf(TEXT("CachePaths() took %.2f Seconds"), appSeconds() - startSeconds);
}
...
const TArray<FString>& appGetKnownLanguageExtensions()
{
	...
	if( KnownLanguageExtensions.Num() == 0 )
	{
		...
		KnownLanguageExtensions.AddItem( TEXT("INT") );
		// @todo dingo: This check is here so we can use coalesced_int.bin, because we don't know yet
		// if we will want to cook, use coalesced.ini's, etc etc
		if (GSys)
		{
 ...
 */
void appSetCommandline(const TCHAR* NewCommandLine)
{
...
void appInit( const TCHAR* InCmdLine, FOutputDevice* InLog, FOutputDeviceConsole* InLogConsole, FOutputDeviceError* InError, FFeedbackContext* InWarn, FFileManager* InFileManager, FCallbackEventObserver* InCallbackEventDevice, FCallbackQueryDevice* InCallbackQueryDevice, FConfigCacheIni*(*ConfigFactory)() )
{
...
#if PLATFORM_64BITS
	#if SHIPPING_PC_GAME
		debugf( NAME_Init, TEXT("Compiled (64-bit - Shipping): %s %s"), ANSI_TO_TCHAR(__DATE__), ANSI_TO_TCHAR(__TIME__) );
	#elif _DEBUG
		debugf( NAME_Init, TEXT("Compiled (64-bit - Debug): %s %s"), ANSI_TO_TCHAR(__DATE__), ANSI_TO_TCHAR(__TIME__) );
	#else 
		debugf( NAME_Init, TEXT("Compiled (64-bit - Release): %s %s"), ANSI_TO_TCHAR(__DATE__), ANSI_TO_TCHAR(__TIME__) );
	#endif
#else
	#if SHIPPING_PC_GAME
		debugf( NAME_Init, TEXT("Compiled (32-bit - Shipping): %s %s"), ANSI_TO_TCHAR(__DATE__), ANSI_TO_TCHAR(__TIME__) );
	#elif _DEBUG
		debugf( NAME_Init, TEXT("Compiled (32-bit - Debug): %s %s"), ANSI_TO_TCHAR(__DATE__), ANSI_TO_TCHAR(__TIME__) );
	#else 
		debugf( NAME_Init, TEXT("Compiled (32-bit - Release): %s %s"), ANSI_TO_TCHAR(__DATE__), ANSI_TO_TCHAR(__TIME__) );
	#endif
#endif
...
#if !CONSOLE
	debugf( NAME_Init, TEXT("Executable: %s"), appExecutableName() );
#endif
	...
	// Outlast Retail Package Patch - honor -SaveAsRetail on the command line so the make
	// and cook commandlets (separate processes the editor menu can't reach) also stamp and
	// serialize packages at the retail version. See UnObjVer.h / ULinkerSave.
	//
	// -SaveAsRetail=MyMod,MyOtherMod  restricts it to those packages (the usual case).
	// -SaveAsRetail                   applies to every package this process saves.
	//
	// bShouldStopOnComma must be FALSE or Parse() would truncate the list at the first comma.
	FString RetailPackageList;
	if ( Parse(appCmdLine(), TEXT("SaveAsRetail="), RetailPackageList, FALSE) )
	{
		GbForceRetailPackageVersion = TRUE;
		appSetRetailPackageFilter( *RetailPackageList );
		warnf( TEXT("Outlast Retail Package Patch: -SaveAsRetail active for package(s) '%s'; saving at %d / %d / engine %d."),
			*RetailPackageList, GForcedPackageFileVersion, GForcedPackageLicenseeVersion, GForcedEngineVersion );
	}
	else if ( ParseParam(appCmdLine(), TEXT("SaveAsRetail")) )
	{
		GbForceRetailPackageVersion = TRUE;
		appSetRetailPackageFilter( NULL );
		warnf( TEXT("Outlast Retail Package Patch: -SaveAsRetail active for ALL packages; saving at %d / %d / engine %d."),
...
static UBOOL appCompressMemoryLZ4( void* CompressedBuffer, INT& CompressedSize, const void* UncompressedBuffer, INT UncompressedSize )
{
	CompressedSize = LZ4_compress((const char*)UncompressedBuffer, (char*)CompressedBuffer, UncompressedSize);
	return CompressedSize > 0;
}

static UBOOL appUncompressMemoryLZ4( void* UncompressedBuffer, INT UncompressedSize, const void* CompressedBuffer, INT CompressedSize )
{
	int Return = LZ4_decompress_fast((const char*)CompressedBuffer, (char*)UncompressedBuffer, UncompressedSize);
	return Return == CompressedSize;
}
...
UBOOL appCompressMemory( ECompressionFlags Flags, void* CompressedBuffer, INT& CompressedSize, const void* UncompressedBuffer, INT UncompressedSize )
{
	...
	check(Flags & (COMPRESS_ZLIB | COMPRESS_LZO | COMPRESS_LZX | COMPRESS_LZ4));
	...
	switch(Flags & COMPRESSION_FLAGS_TYPE_MASK)
	{
		...
		case COMPRESS_LZ4:
#if WITH_LZ4
			bCompressSucceeded = appCompressMemoryLZ4(CompressedBuffer, CompressedSize, UncompressedBuffer, UncompressedSize);
			break;
#endif
		default:
...
UBOOL appUncompressMemory( ECompressionFlags Flags, void* UncompressedBuffer, INT UncompressedSize, const void* CompressedBuffer, INT CompressedSize, UBOOL bIsSourcePadded /*= FALSE*/ )
{
...
#if DINGO
	// mathieu.gauthier: Textures directly write to write-combined
	// memory which is not CPU cached. Since most compression algorithms
	// will often re-use output data, reading from this memory is 
	// extremely slow. So we will uncompress to a temp CPU-cached buffer
	// and do a big memcpy to the final location.
	void* RealUncompressedBuffer = UncompressedBuffer;
	UncompressedBuffer = appMalloc(UncompressedSize);
#endif
	...
	switch(Flags & COMPRESSION_FLAGS_TYPE_MASK)
	{
		...
		case COMPRESS_LZ4:
#if WITH_LZ4
			bUncompressSucceeded = appUncompressMemoryLZ4(UncompressedBuffer, UncompressedSize, CompressedBuffer, CompressedSize);
			break;
#endif
		default:
...
#if DINGO
	appMemcpy(RealUncompressedBuffer, UncompressedBuffer, UncompressedSize);
...
//@zombie_ps4_begin
		case UE3::PLATFORM_Orbis:
			return TEXT("Orbis");
//@zombie_ps4_end

		case UE3::PLATFORM_Dingo:
			return TEXT("Dingo");
...
{
//@zombie_ps4_begin
	return TEXT("xbox360|pc|pcserver|pcconsole|ps3|linux|macosx|iphone|android|ngp|wiiu|orbis");
//@zombie_ps4_end
}
...
//@zombie_ps4_begin
	else if (PlatformStr == TEXT("orbis"))
	{
		return UE3::PLATFORM_Orbis;
	}
//@zombie_ps4_end
	...
	else if (PlatformStr == TEXT("dingo"))
	{
		return UE3::PLATFORM_Dingo;
	}
	else
	{
		SET_WARN_COLOR(COLOR_RED);
...
void RemoveRelativePathing(FString& Path)
{
	const TCHAR* PathStr = *Path;
	while (appStrnicmp(PathStr, L"..\\", 3) == 0)
	{
		PathStr += 3;
	}
	Path = PathStr;
