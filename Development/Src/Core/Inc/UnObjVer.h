...
enum EUnrealEngineObjectVersion
{
	...
	//-Added Orbis audio support
	VER_ORBIS_COMPRESSED_SOUNDS							= 868,
...
enum EUnrealEngineObjectVersionLicensee
{
	/* Update to the OLAI Factory */
	VER_LIC_AI_FACTORY_UPDATE							= 1,
	/* Updeate 2 to the OLAI Factory */
	VER_LIC_AI_FACTORY_UPDATE_2							= 2,
	/* mathieu.gauthier: Added skin shader, changes shader serialization and we have shipped shaders in the wild. */
	VER_LIC_SKIN_SHADING								= 3,

	VER_LIC_AUTOMATIC_VERSION_PLUS_ONE
};

#define VER_LATEST_ENGINE_LICENSEE							(PREPROCESSOR_ENUM_PROTECT(VER_LIC_AUTOMATIC_VERSION_PLUS_ONE) - 1)

// Cooked packages loaded with an older package version are recooked
#define VER_LATEST_COOKED_PACKAGE							142
...
extern INT			GEngineMatchmakingVersion;		// Version for matchmaking

//
// Outlast Retail Package Patch - selectable package versioning.
//
// Normally packages are saved stamped with the current engine versions
// (GPackageFileVersion / GPackageFileLicenseeVersion / GEngineVersion). When
// GbForceRetailPackageVersion is TRUE, ULinkerSave instead stamps the header AND
// drives object-body serialization at the Forced* versions below, so the whole
// package is written in a format the Outlast retail build can load. This is a toggle
// layered on top of the normal save path - it does not change how the game's own
// (native) content is packaged when the toggle is off.
//
// Outlast retail target: Version 867 / LicenseeVersion 3 / Engine 10907.
// (Licensee 3 matches retail exactly - lossless, and still <= retail so it loads.)
//
#define VER_RETAIL_PACKAGE_FILE		867
#define VER_RETAIL_PACKAGE_LICENSEE	3
#define VER_RETAIL_ENGINE_VERSION		10907

extern UBOOL		GbForceRetailPackageVersion;	// Master toggle. FALSE = normal engine save.
extern INT			GForcedPackageFileVersion;		// Epic file version stamped when toggled on.
extern INT			GForcedPackageLicenseeVersion;	// Licensee version stamped when toggled on.
extern INT			GForcedEngineVersion;			// Engine version stamped when toggled on.

//
// Package name filter. -SaveAsRetail=MyMod,MyOtherMod restricts the retail format to
// just those packages; a bare -SaveAsRetail (or the editor menu toggle) leaves the
// filter empty, which means every package saved by this process. Declared with TCHAR
// rather than FString because UnObjVer.h is included before UnString.h.
//
void	appSetRetailPackageFilter( const TCHAR* PackageList );		// Comma-separated package names.
UBOOL	appShouldSaveAsRetailPackage( const TCHAR* PackageName );	// Toggle AND filter test - the only thing ULinkerSave calls.
